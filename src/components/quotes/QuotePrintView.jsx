...
export default function QuotePrintView({ quote, representative, onClose }) {
  ...
  return (
    <div className="bg-white p-8 max-w-5xl mx-auto print:p-4" style={{ fontFamily: 'Arial, sans-serif' }}>
      ...
      <div className="border border-slate-300 mb-4">
        <div className="flex items-start justify-between p-4 gap-4">
          ...
          <div className="flex-1 border border-slate-300">
            <div className="bg-gray-100 border-b border-slate-300 px-3 py-2">
          ...
          {/* Caixa Orçamento */}
          <div className="border border-slate-300 p-3 text-center min-w-[140px]">
      ...
      {/* Cliente */}
      <div className="border border-slate-300 mb-4">
        <div className="bg-gray-100 border-b border-slate-300 px-2 py-1">
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-slate-300 bg-gray-50">
              <th className="border-r border-slate-200 p-1 text-left">Item</th>
              <th className="border-r border-slate-200 p-1 text-left">Qtde</th>
              <th className="border-r border-slate-200 p-1 text-left">Unid</th>
              <th className="border-r border-slate-200 p-1 text-left">Produto</th>
              <th className="border-r border-slate-200 p-1 text-left">NCM</th>
              <th className="border-r border-slate-200 p-1 text-right">Preço Unit</th>
              <th className="border-r border-slate-200 p-1 text-right">ICMS</th>
              <th className="border-r border-slate-200 p-1 text-right">IPI</th>
              <th className="border-r border-slate-200 p-1 text-right">Valor Total</th>
          ...
          <tbody>
            {quote.items?.map((item, index) => (
              <tr key={index} className="border-b border-slate-200">
                <td className="border-r border-slate-200 p-1 text-center">{index + 1}</td>
                <td className="border-r border-slate-200 p-1 text-right">
                  {item.quantity.toFixed(3)}
                </td>
                <td className="border-r border-slate-200 p-1 text-center">{item.unit.toUpperCase()}</td>
                ...
                <td className="border-r border-slate-200 p-1">{item.ncm}</td>
                <td className="border-r border-slate-200 p-1 text-right">
                  {formatCurrency(item.price_per_kg)}
                </td>
                <td className="border-r border-slate-200 p-1 text-right">{item.icms_rate.toFixed(2)}</td>
                <td className="border-r border-slate-200 p-1 text-right">{item.ipi_rate.toFixed(2)}</td>
                <td className="border-r border-slate-200 p-1 text-right">
      ...
      {/* Valores */}
      <div className="bg-gray-50 border border-slate-300 mb-4 p-3">
        <div className="text-sm text-right space-y-1">
          ...
          <p className="text-base pt-2 mt-1 border-t border-slate-400 font-bold">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="border border-slate-300">
          ...
          <div className="p-2 text-xs space-y-1">
            ...
            {quote.notes && (
              <p className="mt-2 pt-2 border-t border-slate-300">{quote.notes}</p>
            )}
          </div>
        </div>
      </div>
